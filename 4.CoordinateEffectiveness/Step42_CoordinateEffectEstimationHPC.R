...
load(fname)
print(fname)
#load("/n/holyscratch01/onnela_lab/ThienLe/ABCcoordinate/fname")
names(mylist)
theta0 = mylist$theta
data = mylist$data
...
travelout_data = matrix(0,nrow=d,ncol=numbercountries)
for ( i in 1:d){
...
totaltravelers_scene1 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp1$travelregulated[[timestep]]
  totaltravelers_scene1[inbound]  = totaltravelers_scene1[inbound] + sum(tmp[,inbound])
}
}
...
totaltravelers_scene2 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp2$travelregulated[[timestep]]
  totaltravelers_scene2[inbound]  = totaltravelers_scene2[inbound] + sum(tmp[,inbound])
}
}
...
totaltravelers_scene3 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp3$travelregulated[[timestep]]
  totaltravelers_scene3[inbound]  = totaltravelers_scene3[inbound] + sum(tmp[,inbound])
}
}


totaltravelers_scene3_adjusted = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp3$travelregulated[[timestep]]
  totaltravelers_scene3_adjusted[inbound]  = totaltravelers_scene3_adjusted[inbound] + sum(tmp[,inbound]*0.05)
}
}
...
for(countryconsider in 1:numbercountries){
  ...
  threshold = max(infectmean_scene2$max_unobservedinfected[countryconsider]*(1+inflation),1) 
  ##inlation alllow compare to shutdown, use max to make sure things threshold not 0
...
totaltravelers_scene4 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp4$travelregulated[[timestep]]
  totaltravelers_scene4[inbound]  = totaltravelers_scene4[inbound] + sum(tmp[,inbound])
}
}

 

####################Scene 5, big countries 1,2,3,4  require all travel in must be 14 days quarantine
###small countries 5,6,7,8 let things fully open #############
...
totaltravelers_scene5 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp5$travelregulated[[timestep]]
  totaltravelers_scene5[inbound]  = totaltravelers_scene5[inbound] + sum(tmp[,inbound])
}
}


  ###########Travel adjusted 5,6,7,8 with full
  ###1,2,3,4 with 5%

totaltravelers_scene5_adjusted = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp5$travelregulated[[timestep]]
 if (inbound %in% c(1,2,3,4)){
  totaltravelers_scene5_adjusted[inbound]  = totaltravelers_scene5_adjusted[inbound] + sum(tmp[,inbound]*0.05)
} else {
  totaltravelers_scene5_adjusted[inbound]  = totaltravelers_scene5_adjusted[inbound] + sum(tmp[,inbound])
...
############################################################  
  ####################Scene 6, big countries 1,2,3,4  require all travel in must be 14 days quarantine
  ###small countries 5,6,7,8 shutdown #############
...
totaltravelers_scene6 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp6$travelregulated[[timestep]]
  totaltravelers_scene6[inbound]  = totaltravelers_scene6[inbound] + sum(tmp[,inbound])
}
}
...
totaltravelers_scene6_adjusted = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp6$travelregulated[[timestep]]
 if (inbound %in% c(1,2,3,4)){
  totaltravelers_scene6_adjusted[inbound]  = totaltravelers_scene6_adjusted[inbound] + sum(tmp[,inbound]*0.05)
} else {
  totaltravelers_scene6_adjusted[inbound]  = totaltravelers_scene6_adjusted[inbound] + sum(tmp[,inbound])
  ...
  ############################################################  
  #############Scene 7, big countries 1,2,3,4 average control, small countries 5,6,7,8 fully open
  inp7 = inp4 # reused most of the part of inp4, all countries average control, but replace 5 to 8 by fully open
 ###Replace travel in of countries 5 to 8 by full travel in
  traveldata7 = inp7$travelregulated
  traveldataf = inp1$travelregulated #fully open travel data
  for (i in 1:length(traveldata7)){
    tmp= traveldata7[[i]]
    tmpf = traveldataf[[i]]
    tmp[,5:8] = tmpf[,5:8]
    traveldata7[[i]] = tmp
  ...
  inp7$travelregulated = traveldata7
...
totaltravelers_scene7 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp7$travelregulated[[timestep]]
  totaltravelers_scene7[inbound]  = totaltravelers_scene7[inbound] + sum(tmp[,inbound])
}
}
 
  ####################Scene 8, big countries 1,2,3,4  require average control
  ###small countries 5,6,7,8 shutdown #############
 ...
 totaltravelers_scene8 = rep(0,numbercountries)
for(inbound in 1:numbercountries){
for(timestep in 1:length(travelout_regulated)){
  tmp = inp8$travelregulated[[timestep]]
  totaltravelers_scene8[inbound]  = totaltravelers_scene8[inbound] + sum(tmp[,inbound])
}
}
...
##SCENE 1: OPEN FULL, SCENE 2 SHUTDOWN, SCENE 3: ALL 14 DAYS QUARANTINE, SCENE 4: ALL AVERAGE CONTROL : ALL COLLABORATE

########BIG CCOUNTRIES COLLABORATE
##SCENE 5: Big countries 1-4 14 days quarantine, Small countries 5-8 open, SCENE 6: Big countries 1-4 14 days quarantine, Small countries 5-8 shutdown
##SCENE 7: Big countries 1-4 average control, Small countries 5-8 open, SCENE 8: Big countries 1-4 average control, small countries 5-8 shutdown

##########SMALL COUNTRIES COLLABORATE
##SCENE9 : Small countries 5-8 require 14 days quarantine, Big countries 1-4 open, SCENE 10: Small countries 5-8 14 days quarantine, big countries 1-4 shutdown 
##SCENE 11: Small countries 5-8 average control, bigcountries 1-4 open, SCENE 12: Small countries 5-8 average control, 
###big countries 1-4 shutdown


numberscenarios = 8 #
 

travelmatrix = matrix(0,numberscenarios,numbercountries)
...
adjustedindex = c(3,5,6)

travelmatrix1 = travelmatrix
for(i in 1:length(adjustedindex)){
  i1 = adjustedindex[i]
  tmp = get( paste("totaltravelers_scene", i1,"_adjusted", sep="") )
  travelmatrix1[i1,] = tmp
}
...
CI_coordinateeffect = matrix(0,24,numberscenarios)

for(i in 1:numberscenarios){ 
  tmp = get( paste("infectpercentile_scene", i, sep="") ) 
  inboundtravel = travelmatrix[i,countryconsider]
    
    CI_coordinateeffect[1:2,i] = tmp$CI_unobservedinfected_imported_prequarantine[countryconsider,]
   
    if(inboundtravel >0){
    CI_coordinateeffect[3:4,i] = tmp$CI_unobservedinfected_imported_prequarantine[countryconsider,]/inboundtravel
    } else {
    CI_coordinateeffect[3:4,i] = tmp$CI_unobservedinfected_imported_prequarantine[countryconsider,]*0
    }

  
  CI_coordinateeffect[5:6,i] = tmp$CI_unobservedinfected_imported_postquarantine[countryconsider,]
    
    if(inboundtravel >0){
    CI_coordinateeffect[7:8,i] = tmp$CI_unobservedinfected_imported_postquarantine[countryconsider,]/inboundtravel
    } else {
    CI_coordinateeffect[7:8,i] = tmp$CI_unobservedinfected_imported_postquarantine[countryconsider,]*0
    }



    CI_coordinateeffect[9:10,i] = tmp$CI_activeconfirmed_imported[countryconsider,]

    if(inboundtravel >0){
    CI_coordinateeffect[11:12,i] = tmp$CI_activeconfirmed_imported[countryconsider,]/inboundtravel
    } else {
    CI_coordinateeffect[11:12,i] = tmp$CI_activeconfirmed_imported[countryconsider,]*0
    }



  
    CI_coordinateeffect[13:14,i] = tmp$CI_newcases_change[countryconsider,c(3,7)]/P[countryconsider]
    CI_coordinateeffect[15:16,i] = tmp$CI_newcases_change[countryconsider,c(4,8)]

    CI_coordinateeffect[17:18,i] = tmp$CI_activeconfirmed_change[countryconsider,c(3,7)]/P[countryconsider]
    CI_coordinateeffect[19:20,i] = tmp$CI_activeconfirmed_change[countryconsider,c(4,8)]


    CI_coordinateeffect[21:22,i] = tmp$CI_Rtchange[countryconsider,c(3,7)]
    CI_coordinateeffect[23:24,i] = tmp$CI_Rtchange[countryconsider,c(4,8)]


 } 

#######################################
travelinbound_countryconsider = travelmatrix[,countryconsider]
travelinbound_countryconsider_adjusted = travelmatrix1[,countryconsider]

finaloutput = rbind(travelinbound_countryconsider, travelinbound_countryconsider_adjusted, CI_coordinateeffect)

fname = paste('Coordinateeffectestimationcountryconsider_',countryconsider,"_durationpolicy_",policytime,"_inflation_",inflation,"_iteration_",iteration,".txt",sep="")
