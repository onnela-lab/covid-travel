...
######################
#####
numbercountries = 8 # change to any number as we want
...
while(length(thetalist)==0){
  ...
  theta0[c(1,5),] = thetafunction1(2)
  theta0[c(2,6),] = thetafunction2(2)
  theta0[c(3,7),] = thetafunction3(2)
  theta0[c(4,8),] = thetafunction4(2)
  ...
  for(i in 1:numbercountries){
    ...
    d1 = d -42  # before the end of travel duration 42 days
  ...
  r1 = min(r1[-1])
  r2 = (maxinfected_stochastic[5:8]/maxinfected_stochastic[5])^(-1)
  r2 = min(r2[-1])
  r3 = min(r1,r2)
  ###################Make sure Rt for all groups still in the range
  R1 = max((Rt[1] -1.1)*(Rt[1] - 6.47), (Rt[5]-1.1)*(Rt[5]-6.47))
  R2 = max((Rt[2] -1.1)*(Rt[2] - 1), (Rt[6]-1.1)*(Rt[6]-1))
  R3 = max((Rt[3] -1)*(Rt[3] - 0.9), (Rt[7]-1)*(Rt[7]-0.9))
  R4 = max((Rt[4] -0.9)*(Rt[4] - 0.47), (Rt[8]-0.9)*(Rt[8]-0.47))
  R = max(R1,R2,R3,R4)

  ##############
  if (r3 > 5 && sum(checkcountries_stochastic) == numbercountries&& R<0) {
    thetalist = theta0
