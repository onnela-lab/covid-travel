#The code is created on Oct 29 to run on ABC for Traffic mode, 3 countries
...
setwd("C:/Users/thl902/Desktop/CONETPublicationDec27")
...
iteration = 1 # the k-th MCMC iteration
policytime = 7 #use 2 weeks for evaluation
...
prob =prob1/1000
particles = 300 # Need to change to 1000
...
infectedprediction_mean =  function( theta, inp){
  ...
  unobservedinfected = matrix(0,numbercountries,inp$durationtravel) 
  activeconfirmed = matrix(0,numbercountries, inp$durationtravel) 
  ...
  for (country in 1:numbercountries){
    ...
    a5 = 5 + (country -1)*6
    a6 = 6 + (country -1)*6
    #Unbserved infected and active confirmed
    unobservedinfected[country,] = u[,a2]
    activeconfirmed[country,] = u[,a3]
  ...
  output = list(unobservedinfected_imported_prequarantine = unobservedinfected_imported_prequarantine,
                ...
                Rtchange = Rtchange,
                unobservedinfected =  unobservedinfected,
...
infectedprediction_percentile =  function(k, percentile_lower, percentile_upper, theta, inp){
  ...
  CI_unobservedinfected = matrix(0, 2*numbercountries, inp$durationtravel)
  CI_activeconfirmed = matrix(0, 2*numbercountries, inp$durationtravel)
  ...
  for( country in 1:numbercountries){
    ...
    Ipred_list[[country]] = matrix(0, k, inp$durationtravel)
    Apred_list[[country]] = matrix(0, k, inp$durationtravel)
  ...
  for(i in 1:k){
    ...
    for (country in 1:numbercountries){
      ...
      a5 = 5 + (country -1)*6
      a6 = 6 + (country -1)*6
      #undocument infected and active confirmed
      Ipred_list[[country]][i,] = u[,a2]
      Apred_list[[country]][i,] = u[,a3]
  ...
  for(country in 1:numbercountries){
   ...
   CI_unobservedinfected[t1,] = colQuantiles(Ipred_list[[country]], probs = percentile_lower)
                               
   CI_unobservedinfected[t2,] = colQuantiles(Ipred_list[[country]], probs = percentile_upper) 
    
   CI_activeconfirmed[t1,] = colQuantiles(Apred_list[[country]], probs = percentile_lower)
   
   CI_activeconfirmed[t2,] = colQuantiles(Apred_list[[country]], probs = percentile_upper) 
  ...
  output = list(CI_activeconfirmed_imported = CI_activeconfirmed_imported,
                ...
                CI_unobservedinfected = CI_unobservedinfected,
                CI_activeconfirmed = CI_activeconfirmed)
...
dataplot_infectedprediction =  function(k, theta, inp){
  Ipred_list = list()
  Apred_list = list()
  for( country in 1:numbercountries){
    Ipred_list[[country]] = matrix(0, k, inp$durationtravel)
  ...
  for(i in 1:k){
    u1 = stochasticmodel_outadjust_pandemictravel(theta, inp)
    u = u1$model_output
     for (country in 1:numbercountries){
      a1 = 1 + (country -1)*6
      a2 = 2 + (country -1)*6
  ...
  output = list(
                dataplot_unobservedinfected = Ipred_list,
                dataplot_activeconfirmed = Apred_list)
...
dataplot_policy1 = dataplot_infectedprediction(trials, thetahat, inp)
...
dataplot_policy2 = dataplot_infectedprediction(trials, thetahat, inp)
...
#threshold = infectmean_policy2$max_unobservedinfected[countryconsider]*(1+.03) # inlation alllow compare to shutdown
threshold = 70
  ...
  p = min(pseq)
  #####
  print(paste("The proportion is",p) )
  ##############
    ...
    print(paste(c("The capacity divided sequence is",infecttravelin_capacity_divide)), collapse="\t" )
    ...
    for ( country in 1:numbercountries){
      ...
      print(paste("This is", a3))
      print(c(paste("The rate in i", ratein)), collapse="\t")
...
######
  ...
  dataplot_policy5 = dataplot_infectedprediction(trials, thetahat, inp)
  ...
  totaltravelers_policy5/totaltravelers_policy1
############################PLOTTING DIFFERENT METHODS
  #UNDOCUMENT INFECTED
  undocumentinfected_mat =  matrix(0,nrow=inp$durationtravel, ncol=3)
  undocumentinfected_mat[,1]  = infectmean_policy1$unobservedinfected[countryconsider,]
  undocumentinfected_mat[,2]  = infectmean_policy2$unobservedinfected[countryconsider,]
  undocumentinfected_mat[,3]  = infectmean_policy5$unobservedinfected[countryconsider,]
  t = seq(0, inp$durationtravel-1)
  data = data.frame(t,undocumentinfected_mat)
  colnames(data) = c("Time", "Fullyopen", "Borderclosure", "Averagecontrol")
  #data = data[1:20,]
  data1_long = gather(data, Strategies, Cases, Fullyopen:Averagecontrol, factor_key=TRUE)
  
  
sizetext  = 22
sizetext1 = 16
plt1 = ggplot(data1_long, aes(x=Time, y=Cases, color=Strategies)) + 
    geom_line(size=.75)+ geom_hline(yintercept=70, linetype="dashed",size=.75)+
     annotate(geom="text", x=5, y=80, label="y =70", size=7)+theme_classic()+
    labs( x = "Traffic regulating duration (days)", y = "Undetected infected")+
  theme(
      axis.text=element_text(size=sizetext),
      axis.title.x = element_text( size=sizetext, face ="bold"),
      axis.title.y = element_text(size=sizetext, face="bold"), legend.position=c(.4, .6),
...
fname = paste('Averagecontrol1',".pdf",sep="")
ggsave(fname,plot=plt1)
 ...
 ##############
  countryconsider 
  totaltravelers_policy1_12 = 0 
  for(timestep in 2:length(travelout_regulated)){
    tmp = inp1$travelregulated[[timestep]]
    totaltravelers_policy1_12 = totaltravelers_policy1_12 + sum(tmp[1,countryconsider])
  }
  ########
  totaltravelers_policy1_32 = 0 
  for(timestep in 2:length(travelout_regulated)){
    tmp = inp1$travelregulated[[timestep]]
    totaltravelers_policy1_32 = totaltravelers_policy1_32 + sum(tmp[3,countryconsider])
  }
  ###########
  
  countryconsider 
  totaltravelers_policy5_12 = 0 
  for(timestep in 2:length(travelout_regulated)){
    tmp = inp5$travelregulated[[timestep]]
    totaltravelers_policy5_12 = totaltravelers_policy5_12 + sum(tmp[1,countryconsider])
  }
  ########
  totaltravelers_policy5_32 = 0 
  for(timestep in 2:length(travelout_regulated)){
    tmp = inp5$travelregulated[[timestep]]
    totaltravelers_policy5_32 = totaltravelers_policy5_32 + sum(tmp[3,countryconsider])
  }
  #################
  
  totaltravelers_policy5_32/totaltravelers_policy1_32
  totaltravelers_policy5_12/totaltravelers_policy1_12
  (totaltravelers_policy5_32 + totaltravelers_policy5_12)/(totaltravelers_policy1_32+totaltravelers_policy1_12)
  
  
############ ACTIVE CONFIRMED
  
#   activeconfirmed_mat =  matrix(0,nrow=inp$durationtravel, ncol=3)
#   activeconfirmed_mat[,1]  = infectmean_policy1$activeconfirmed[countryconsider,]
#   activeconfirmed_mat[,2]  = infectmean_policy2$activeconfirmed[countryconsider,]
#   activeconfirmed_mat[,3]  = infectmean_policy5$activeconfirmed[countryconsider,]
#   t = seq(1, inp$durationtravel)
#   data = data.frame(t,activeconfirmed_mat)
#   colnames(data) = c("Time", "Fulltravel", "Shutdown", "Averagecontrol")
#   #data = data[1:20,]
#   data1_long = gather(data, Strategies, Cases, Fulltravel:Averagecontrol, factor_key=TRUE)
#   
#   plt2 = ggplot(data1_long, aes(x=Time, y=Cases, color=Strategies)) +
#     geom_line()+labs(title = "Prediction for mean of activeconfirmed during the regulating time by different strategies")
#   plt2 
#   
#   
#   
#   
#   ########CONFIDENCE INTERVAL, SAVE FOR LATER USE
#   
#   undocumentinfected_mat =  matrix(0,nrow=inp$durationtravel, ncol=3)
#   undocumentinfected_mat[,1]  = infectpercentile_policy1$CI_unobservedinfected[4,]
#   undocumentinfected_mat[,2]  = infectpercentile_policy2$CI_unobservedinfected[4,]
#   undocumentinfected_mat[,3]  = infectpercentile_policy5$CI_unobservedinfected[4,]
#   t = seq(1, inp$durationtravel)
#   data = data.frame(t,undocumentinfected_mat)
#   colnames(data) = c("Time", "Fulltravel", "Shutdown", "Averagecontrol")
#   #data = data[1:20,]
#   data1_long = gather(data, Strategies, Cases, Fulltravel:Averagecontrol, factor_key=TRUE)
#   
#   plt3 = ggplot(data1_long, aes(x=Time, y=Cases, color=Strategies)) +
#     geom_line()+labs(title = "Prediction for mean of undocument infected during the regulating time by different strategies")
#   plt3 + geom_hline(yintercept=70, linetype="dashed", color = "red")+
#     annotate(geom="text", x=6, y=77, label="Threshold c =70",
#              color="blue")
#   
#   
# ############################ Probability control
...
#       
#       print(paste("This is", a3))
#       print(c(paste("The rate in probability control i", ratein)), collapse="\t")
#       
#     }
#     
#     travelout_regulated = totaltravelout_regulated(travelprediction, travelrate_matrix, P) #total travelers regulated by policy 3
...
#   
#   dataplot_policy7 = dataplot_infectedprediction(trials, thetahat, inp)
#   
...
#   undocumentinfected_mat[,1]  = infectmean_policy1$unobservedinfected[countryconsider,]
#   undocumentinfected_mat[,2]  = infectmean_policy2$unobservedinfected[countryconsider,]
#   undocumentinfected_mat[,3]  = infectmean_policy7$unobservedinfected[countryconsider,]
#   t = seq(1, inp$durationtravel)
#   data = data.frame(t,undocumentinfected_mat)
#   colnames(data) = c("Time", "Fulltravel", "Shutdown", "Probabilitycontrol")
#   #data = data[1:20,]
#   data1_long = gather(data, Strategies, Cases, Fulltravel:Probabilitycontrol, factor_key=TRUE)
#   
#   plt1 = ggplot(data1_long, aes(x=Time, y=Cases, color=Strategies)) +
#     geom_line()+labs(title = "Prediction for mean of undocument infected during the regulating time by different strategies")
#  
#    plt1 + geom_hline(yintercept=70, linetype="dashed", color = "red")+
#     annotate(geom="text", x=6, y=77, label="Threshold c =70",
#              color="blue")
#   
#   
...
#   undocumentinfected_mat[,2]  = infectpercentile_policy2$CI_unobservedinfected[4,]
#   
#   undocumentinfected_mat[,3]  = colQuantiles(dataplot_policy7$dataplot_unobservedinfected[[2]], probs = .8)
#   #######
#   h = apply(dataplot_policy7$dataplot_unobservedinfected[[2]],2,max)
#   summary(h)
#   
#   t = seq(1, inp$durationtravel)
#   data = data.frame(t,undocumentinfected_mat)
...
#   plt3 = ggplot(data1_long, aes(x=Time, y=Cases, color=Strategies)) +
#     geom_line()+labs(title = "Prediction for mean of undocument infected during the regulating time by different strategies")
#   plt3 + geom_hline(yintercept=400, linetype="dashed", color = "red")+
#     annotate(geom="text", x=6, y=77, label="Threshold c =70",
#              color="blue")
#   
#   
