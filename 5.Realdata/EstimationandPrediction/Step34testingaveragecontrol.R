...
# load("flightdataJanJune20mixed19.Rdata")
# load("InitialStep3.Rdata")
...
inp = inp1
names(inp)
##TOTAL TRAVEL OUT AS 2019 EACH DAY FROM EACH COUNTRY#######
travelprediction = matrix(0,inp$durationtravel, numbercountries)
for(i in 1:length(inp$travelregulated)){
  tmp = as.matrix(inp$travelregulated[[i]])
  travelprediction[i,] = rowSums(tmp)
}
