setwd("C:/Users/thl902/Desktop/CONETPublicationDec27/Simulation/1.Estimation1country")
library(ggplot2)
library(cowplot)
particles = 1000
prob=.01
...
texoutfun1(1000,.01,0)# median, 5 to 8, bias

texoutfun2(1000,.01,0)# median, 5 to 8, relative bias
...
p1a <- ggplot(long_mydat1, aes(x=Parameter, y=Value, fill=Parameter)) +
  geom_boxplot()+ 
  labs( x = "Euclidean distance", y = "")+ theme_classic()+ ylim(0, 0.5)+theme(
    axis.text=element_text(size=9),
    axis.title.x = element_text( size=9, face ="bold"),
    axis.title.y = element_text(size=9, face="bold"), legend.position="none",
...
p4a <- ggplot(long_mydat4, aes(x=Parameter, y=Value, fill=Parameter)) +
  geom_boxplot()+ 
  labs( x = "Proposed distance", y = "")+ theme_classic()+ ylim(0, 0.5)+theme(
    axis.text=element_text(size=9),
    axis.title.x = element_text( size=9, face ="bold"),
    axis.title.y = element_text(size=9, face="bold"), legend.position=c(.8,.8),
    legend.title = element_text( size=9, 
...
p1b = p1a + theme(plot.margin = unit(c(1, 0, 1, 1), "lines"))
p4b <- p4a + theme(plot.margin = unit(c(1, 1, 1, -1), "lines"))




#grid.arrange(p1a, p4a,ncol=2)

plte = grid.arrange(p1b, p4b,ncol=2)


  
fname1 = paste('IQRcompare',".pdf",sep="")
